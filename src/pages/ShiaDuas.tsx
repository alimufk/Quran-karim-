...
export function ShiaDuas() { 
  ...
  // حالة جديدة لإظهار إشعارات تحميل الخلفية للمستخدم بصورة ذكية
  const [toastMessage, setToastMessage] = useState<string | null>(null);

  // إخفاء الإشعار العائم تلقائياً بعد 4 ثوانٍ
  useEffect(() => {
    if (toastMessage) {
  ...
  // 🔥 دالة التحميل في الخلفية الاحترافية والغير حاصرة للواجهة نهائياً
  const handleDeviceDownload = (e: any, url: string, name: string) => {
    e.stopPropagation(); 
    setErrorMessage(null);

    if (url.includes('YOUR_FILE_NAME') || url.endsWith('/audio/')) {
      setErrorMessage(`رابط (${name}) غير جاهز بعد! يرجى وضع الرابط الفعلي في ملف السيرفر.`);
      return;
    }

    // 1. إعلام المستخدم فوراً بأن التحميل بدأ في الخلفية لضمان تجربة سلسة
    setToastMessage(`جاري تحميل ملف (${name}) في الخلفية... يمكنك الاستمرار بالتصفح`);

    // 2. إطلاق خط التحميل بشكل منفصل تماماً (Non-blocking Background Thread)
    fetch(url)
      .then(response => {
        if (!response.ok) {
          if (response.status === 404) {
            setErrorMessage(`الملف (${name}) غير موجود على السيرفر (404). تأكد من مطابقة الاسم المكتوب في الكود لـ GitHub.`);
      ...
      .then(blob => {
        ...
        // تحويل البيانات وحفظها في جهاز المستخدم فور جهوزيتها بالخلفية
        ...
        // إشعار نجاح التحميل النهائي
        setToastMessage(`✅ تم اكتمال تحميل (${name}) وحفظه في جهازك.`);
      })
      .catch(() => {
        // حماية احتياطية في حال تعطل بروتوكولات الأمان بالمتصفح لإتمام العملية
        try {
          const link = document.createElement('a');
          link.href = url;
          link.download = `${name}.mp3`;
  ...
  return ( 
    <div className="flex flex-col h-full bg-[#022c22] relative"> 
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} key={dua.id} > 
            <div onClick={() => handlePlayToggle(dua.id)} className={`flex items-center justify-between p-4 rounded-[24px] border cursor-pointer transition-all ${currentDuaId === dua.id ? 'bg-[#059669]/30 border-[#fbbf24]/50' : 'border-[#059669]/20 bg-[#064e3b]/40 hover:bg-[#059669]/30 hover:border-[#059669]/50'}`} > 
              ...
              <div className="flex items-center gap-2"> 
                ...
                {/* زر تحميل دائم وثابت للجهاز - يعمل فورا بالخلفية بدون تعطيل الكبسة */}
                <button 
                  onClick={(e) => handleDeviceDownload(e, dua.url, dua.name)} 
                  className="p-2 text-[#fbbf24] hover:bg-[#fbbf24]/10 rounded-full transition active:scale-90" 
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-[#064e3b] px-6 py-5 border-t border-[#059669]/50 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] rounded-t-[32px] z-50"> 
          <div className="flex justify-between items-center"> 
            ...
            <div className="flex items-center gap-4"> 
              ...
              {/* زر التحميل بالشريط السفلي يعمل فورا بالخلفية */}
              <button 
                onClick={(e) => handleDeviceDownload(e, currentDua.url, currentDua.name)} 
                className="p-3 text-[#fbbf24] hover:bg-[#fbbf24]/10 rounded-full transition-colors active:scale-90" 
                title="تحميل الملف للجهاز" 
              > 
                <Download size={22} /> 
              </button>
      ...
      {/* واجهة إشعار عائم Toast احترافي يظهر بالأسفل عند تحميل الخلفية */}
      <AnimatePresence>
        {toastMessage && (
          <motion.div 
