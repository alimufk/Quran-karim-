...
export function Settings() {
    ...
    // --- دالة المشاركة الذكية المحدثة ---
    const handleShareApp = async () => {
        const shareTitle = 'تطبيق القرآن الكريم';
        const shareText = 'حمل الآن تطبيق القرآن الكريم الشامل (أوقات الصلاة، الأذكار، دليل الزائرين ومساعد الذكاء الاصطناعي).';
        const shareUrl = 'https://alimufk.github.io/saeesaee1985/';

        // 1. محاولة استخدام مشاركة النظام الأصلية
        if (navigator.share && window.location.protocol === 'https:') {
            try {
                ...
                console.log('تم إلغاء المشاركة الأصلية');
            }
        }

        // 2. الحل البديل إذا لم يدعم المتصفح المشاركة الأصلية
        const confirmShare = window.confirm("هل تريد مشاركة رابط التطبيق مباشرة عبر الواتساب؟\n\n(إذا اخترت إلغاء، سيتم نسخ الرابط تلقائياً للحافظة)");
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex flex-col justify-end">
                        <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="bg-[#0e0e10] border-t border-zinc-800 rounded-t-[32px] max-h-[80vh] flex flex-col pb-12">
                            <div className="sticky top-0 bg-[#0e0e10] z-10 px-6 py-5 border-b border-zinc-800/40 flex justify-between items-center">
                                ...
                                <button onClick={() => setActiveModal(null)} className="px-4 py-1.5 bg-zinc-900 hover:bg-zinc-800 text-zinc-300 text-xs font-bold rounded-full transition-all">إغلاق</button>
                            </div>
